/*
** EPITECH PROJECT, 2024
** my_paint
** File description:
** my_paint.c
*/

#include "my_paint.h"

void create_about(my_paint_t *my_paint)
{
    sfTexture* texture = sfTexture_createFromFile("assets/about.png", NULL);
    sfSprite* sprite = sfSprite_create();
    sfVector2u window_size = sfRenderWindow_getSize(my_paint->window.window);
    sfVector2u texture_size = sfTexture_getSize(texture);
    float scale_x = 0.8f * window_size.x / texture_size.x;
    float scale_y = 0.8f * window_size.y / texture_size.y;
    float pos_x;
    float pos_y;

    sfSprite_setTexture(sprite, texture, sfTrue);
    pos_x = (window_size.x - scale_x * texture_size.x) / 2;
    pos_y = (window_size.y - scale_y * texture_size.y) / 2;
    sfSprite_setPosition(sprite, (sfVector2f){pos_x, pos_y});
    my_paint->about.sprite = sprite;
    my_paint->about.texture = texture;
}

void display_about(my_paint_t *my_paint)
{
    if (my_paint->about.display_about == 1) {
        update_about_position(my_paint);
        sfRenderWindow_drawSprite(my_paint->window.window,
        my_paint->about.sprite, NULL);
    }
}

static void update_about_position(my_paint_t *my_paint)
{
    sfVector2u window_size = sfRenderWindow_getSize(my_paint->window.window);
    sfVector2f about_size = {window_size.x * 0.8f, window_size.y * 0.8f};
    sfVector2f about_position = {window_size.x / 2.0f - about_size.x / 2.0f,
        window_size.y / 2.0f - about_size.y / 2.0f};

    sfSprite_setPosition(my_paint->about.sprite, about_position);
}

void about_btn_act(my_paint_t *my_paint, button_t *button)
{
    if (my_paint->about.display_about == 0)
        my_paint->about.display_about = 1;
    else
        my_paint->about.display_about = 0;
}
