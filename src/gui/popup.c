...
static void initialize_popup(my_paint_t *my_paint)
{
    ...
    my_paint->window.popup = create_popup_rectangle(popupSize,
                                                    popupPosition, color);
    my_paint->window.popup_text = create_popup_text(textPosition, font,
        24, sfWhite);
}

static void cleanup_popup(my_paint_t *my_paint)
{
    sfRectangleShape_destroy(my_paint->window.popup);
    sfText_destroy(my_paint->window.popup_text);
    free(my_paint->window.popup_text_str);
}
...
int create_popup(my_paint_t *my_paint)
{
    ...
    if (my_paint->window.popup_text_str == NULL) {
